"""
Dynamic Portfolio Backtester — US Market ($6,000)
AmiBroker-style: daily re-rank all US stocks by ADX+SMA20 trend strength
"""
...
# ── CONFIG ────────────────────────────────────────────
CAPITAL = 6000                   # USD
MAX_POSITIONS = 6                # Reduced from 10 — less concurrent exposure
RISK_PCT = 1.0
COMMISSION = 0.001
PERIOD = "7y"
SL_MULTIPLE = 2.8
...
BB_PERIOD = 20
TP_RATIO = 0.4                   # 0.4R
MIN_ADX = 25                     # Increased from 20 — stricter trend filter
MIN_TREND_SCORE = 30             # Increased from 0 — only top trending stocks
...
def print_results(result, capital):
    print()
    print("=" * 65)
    print("  DYNAMIC PORTFOLIO — US MARKET RESULTS")
    print("=" * 65)
...
def plot_results(result):
    try:
        ...
        ax1.set_title('US Dynamic Portfolio — Equity Curve', color='#d4af37', fontsize=14, fontweight='bold')
        ...
        plt.tight_layout()
        chart_path = os.path.join(OUTPUT_DIR, 'dynamic_us_chart.png')
...
if __name__ == '__main__':
    print("=" * 65)
    print("  DYNAMIC PORTFOLIO — US MARKET")
    ...
    with open(csv_path) as f:
        ...
        for row in reader:
            ...
            if not sym.startswith('^'):  # Skip index tickers
                tickers.append(sym)
