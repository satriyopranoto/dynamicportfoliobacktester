...
print(f"Tickers loaded: {len(tickers_list)}")
...
def check_buy_signal(stock, bar_idx, multi_tf=False):
    ...
    if multi_tf:
        wk_sma = float(stock['wk_sma'][bar_idx]) if not np.isnan(stock['wk_sma'][bar_idx]) else None
...
def run_dynamic_portfolio(stocks_dict, multi_tf=False):
    ...
    for day_idx, today in enumerate(timeline):
        if day_idx % 500 == 0:
            print(f"    Day {day_idx}/{len(timeline)}... ({len(positions)} open, Rp{cash:,.0f})")
        ...
        if len(positions) < MAX_POSITIONS:
            ...
            for cand in candidates:
                ...
                positions.append({'ticker':cand['ticker'],'entry_price':cand['close'],
                    'entry_sl':cand['sl_val'],'size':size,'cost':cost,'entry_date':today,
    ...
    if len(all_trades) > 0:
        ...
        returns['cagr'] = ((final_eq/CAPITAL)**(1/yrs)-1)*100 if yrs>0 else 0
    
    return returns, equity_history, timeline, all_trades

# ══ MAIN ══
print("="*60)
print("  DYNAMIC PORTFOLIO — ID MARKET (10 Years, 116 Stocks)")
print("  Basis+ADX: Single TF vs Multi TF (Daily+Weekly)")
print(f"  Capital: Rp{CAPITAL:,}, Max Pos: {MAX_POSITIONS}")
print("="*60)

print(f"\nLoading {len(tickers_list)} IDX stocks...")
stocks = {}
