"""
IDX — Intraday 1h + Daily Multi-TF
Compare: Standard vs Multi-TF (daily PDI>MDI) vs Modif (daily PDI>MDI + PDI>PDI[1])
1 year data
"""
import os, sys, csv, numpy as np, pandas as pd, yfinance as yf, warnings
warnings.filterwarnings('ignore')

CAPITAL = 100_000_000; MAX_POS = 10; RISK_PCT = 1.0; MIN_ADX = 20
PERIOD = "1y"
...
with open(csv_path) as f:
    ...
    tickers = [row['Symbol'].strip().upper().replace('.JK','')+'.JK' for row in reader]
...
def load(ticker):
    try:
        ...
        c1 = h1['Close'].values.astype(float)
        hh1 = h1['High'].values.astype(float)
        l1 = h1['Low'].values.astype(float)
        ...
        r = {'ticker':ticker,'dates':h1.index,'close':c1,'high':hh1,'low':l1,
             'sma20':sma20,'sl':slv,'adx':adx,'pdi':pdi,'mdi':mdi}
        ...
        # Daily PDI 1-ago (for modif rising check)
        dp_1ago = np.full_like(dp, np.nan)
        dm_1ago = np.full_like(dm, np.nan)
        if len(dp) > 1:
            dp_1ago[1:] = dp[:-1]
            dm_1ago[1:] = dm[:-1]
        ...
        h1_dates = h1.index
        hp = np.full(len(h1_dates), np.nan); hm = np.full(len(h1_dates), np.nan)
        hp1 = np.full(len(h1_dates), np.nan); hm1 = np.full(len(h1_dates), np.nan)
        for i, dt in enumerate(h1_dates):
            w = get_d(dt)
            hp[i] = w.get('pdi', np.nan); hm[i] = w.get('mdi', np.nan)
...
def run(stocks, mode=0):
    ...
    tl = sorted(all_d)
    print(f"  Bars: {len(tl)} | {tl[0].date()} → {tl[-1].date()}")
    si = {}
    for t, s in stocks.items():
        im = {}
    ...
    fe = eq[-1] if eq else CAPITAL; yrs = len(tl)/(252*7)
    ...
    print(f"  Return: {r['ret']:+.2f}% | DD: -{r.get('dd',0):.2f}% | Sharpe: {r.get('sharpe',0):.2f}")
...
labels = {0: "SINGLE TF", 1: "MULTI-TF", 2: "MODIF"}
print(f"\n\n{'='*60}")
print(f"  📊 PERBANDINGAN — IDX Intraday 1h")
print(f"{'='*60}")
