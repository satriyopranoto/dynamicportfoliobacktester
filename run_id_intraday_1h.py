"""
IDX — Intraday 1h + Daily Multi-TF Moderat
Basis+ADX Entry on 1h with daily PDI_d > MDI_d confirmation
1 year data
"""
...
def run(stocks, multi=False):
    ...
    fe = eq[-1] if eq else CAPITAL; yrs = len(tl)/(252*7)  # ~7 bars per day for 1h
...
for multi in [False, True]:
    ...
    print(f"  CAGR     : {r.get('cagr',0):+.2f}%/yr" if r.get('cagr') else "")
    print(f"  Sharpe   : {r.get('sharpe',0):.2f}")
    print(f"  Max DD   : -{r.get('dd',0):.2f}%")
    print(f"  Trades   : {r['n']} | Win Rate: {r.get('wr',0):.1f}%")
